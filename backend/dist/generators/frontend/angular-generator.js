...
exports.generateAngularFrontend = generateAngularFrontend;
...
export class AppComponent {
  readonly appTitle = ${(0, naming_1.quoteJs)(schema.appTitle)};
  readonly links = [
    { label: 'Demo', path: '/demo' },
    ${schema.tables.map((table) => `{ label: ${(0, naming_1.quoteJs)(table.className)}, path: '/${table.routePath}' }`).join(',\n    ')},
...
export class DemoPage {
  readonly screens = [
    ${schema.tables.map((table) => `{ name: ${(0, naming_1.quoteJs)(table.className)}, route: '/${table.routePath}', description: 'CRUD pour la table ${table.name}' }`).join(',\n    ')}
  ];
}
`;
}
...
async function generateAngularFrontend(schema, root) {
    ...
    await (0, file_writer_1.writeTextFile)(path_1.default.join(frontendRoot, 'angular.json'), angularJson());
    ...
    await (0, file_writer_1.writeTextFile)(path_1.default.join(frontendRoot, 'src/app/app.config.ts'), appConfig());
    ...
    await (0, file_writer_1.writeTextFile)(path_1.default.join(frontendRoot, 'src/app/app.component.html'), appShell());

...
import { NormalizedColumn, NormalizedSchema, NormalizedTable } from '../../core/types';
...
function angularJson(schema: NormalizedSchema): string {
  return json({
...
function appShell(schema: NormalizedSchema): string {
...
export class AppComponent {
  constructor() {
    ...
    document.title = ${quoteJs(schema.appTitle)};
  }
}
`;
}
...
function emptyValue(column: NormalizedColumn): string {
  if (column.tsType === 'number') return 'null';
  if (column.tsType === 'boolean') return 'false';
  return "''";
}
...
export async function generateIonicFrontend(schema: NormalizedSchema, root: string): Promise<void> {
  const frontendRoot = path.join(root, 'frontend');

  await writeTextFile(path.join(frontendRoot, 'package.json'), packageJson(schema));
  await writeTextFile(path.join(frontendRoot, 'angular.json'), angularJson(schema));
  ...
  await writeTextFile(path.join(frontendRoot, 'src/app/app.component.html'), appShell(schema));

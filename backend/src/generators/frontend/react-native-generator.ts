...
import { NormalizedSchema } from '../../core/types';
...
function appJson(schema: NormalizedSchema): string {
  return json({
    expo: {
...
function tableMeta(schema: NormalizedSchema): string {
...
export async function generateReactNativeFrontend(schema: NormalizedSchema, root: string): Promise<void> {
  const frontendRoot = path.join(root, 'frontend');

  await writeTextFile(path.join(frontendRoot, 'package.json'), packageJson(schema));
  await writeTextFile(path.join(frontendRoot, 'app.json'), appJson(schema));
  await writeTextFile(path.join(frontendRoot, 'tsconfig.json'), tsConfig());
  await writeTextFile(path.join(frontendRoot, 'App.tsx'), appTsx(schema));
  await writeTextFile(path.join(frontendRoot, 'src/api.ts'), apiClient());
  await writeTextFile(path.join(frontendRoot, 'src/tables.ts'), tableMeta(schema));
  await writeTextFile(path.join(frontendRoot, 'src/screens/TableScreen.tsx'), tableScreen());
}

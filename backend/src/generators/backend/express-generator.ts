...
import { quoteJs, toCamelCase } from '../../core/naming';
...
function envExample(schema: NormalizedSchema): string {
...
function dbFile(schema: NormalizedSchema): string {
...
function routeFile(table: NormalizedTable): string {
  return `
...
function serverFile(schema: NormalizedSchema): string {
  const imports = schema.tables
    .map((table) => `import ${toCamelCase(table.name)}Routes from './routes/${table.entityName}.routes.js';`)
    .join('\n');
  const routeUses = schema.tables
    .map((table) => `app.use('/api/${table.routePath}', ${toCamelCase(table.name)}Routes);`)
    .join('\n');
...
function authServiceFile(): string {
  return `
...
function authControllerFile(): string {
  return `
...
export async function generateExpressBackend(schema: NormalizedSchema, root: string): Promise<void> {
  const backendRoot = path.join(root, 'backend');

  await writeTextFile(path.join(backendRoot, 'package.json'), packageJson(schema));
  await writeTextFile(path.join(backendRoot, '.env.example'), envExample(schema));
  await writeTextFile(path.join(backendRoot, 'src/db.js'), dbFile(schema));
  await writeTextFile(path.join(backendRoot, 'src/server.js'), serverFile(schema));
  await writeTextFile(path.join(backendRoot, 'src/services/auth.service.js'), authServiceFile());
  await writeTextFile(path.join(backendRoot, 'src/controllers/auth.controller.js'), authControllerFile());
  await writeTextFile(path.join(backendRoot, 'src/routes/auth.routes.js'), authRouteFile());

  for (const table of schema.tables) {
    await writeTextFile(
      path.join(backendRoot, `src/services/${table.entityName}.service.js`),
      serviceFile(schema, table)
    );
    await writeTextFile(
      path.join(backendRoot, `src/controllers/${table.entityName}.controller.js`),
      controllerFile(table)
    );
    await writeTextFile(
      path.join(backendRoot, `src/routes/${table.entityName}.routes.js`),
      routeFile(table)
    );
  }
}
